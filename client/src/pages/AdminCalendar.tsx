...
const TYPE_CONFIG: Record<EventType, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  call:    { label: "Call",    color: `${BRAND.emerald}`, icon: <Phone size={11} /> },
...
const STATUS_CONFIG: Record<EventStatus, { label: string; color: string }> = {
  ...
  confirmed:  { label: "Confirmed",  color: `${BRAND.emerald}` },
      ...
      <div style={{ marginTop: "0.75rem", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.625rem", background: "rgba(255,255,255,0.025)", overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "1rem", padding: "1rem", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ display: "flex", gap: "0.375rem", alignItems: "flex-start", flexWrap: "wrap", justifyContent: "flex-end" }}>
            {ev.shareToken && (
              <button onClick={() => copyShareLink(ev.shareToken!)} title="Copy share link" style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "0.25rem", padding: "0.25rem 0.375rem", cursor: "pointer", color: copiedToken === ev.shareToken ? `${BRAND.emerald}` : "#64748b", display: "flex", alignItems: "center" }}>
                {copiedToken === ev.shareToken ? <Check size={12} /> : <Copy size={12} />}
              </button>
            )}
            {ev.status === "scheduled" && (
              <button onClick={() => { setConfirmingId(ev.id); confirmMutation.mutate({ id: ev.id }); }} title="Mark Confirmed" style={{ background: "transparent", border: `1px solid ${emeraldAlpha(0.3)}`, borderRadius: "0.25rem", padding: "0.25rem 0.375rem", cursor: "pointer", color: confirmingId === ev.id ? `${BRAND.emerald}` : "#4ade80", display: "flex", alignItems: "center", gap: "0.2rem", fontSize: "0.5625rem" }}>
  ...
  if (!user) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#0a0a0a" }}>
        <a href={getLoginUrl()} style={{ color: `${BRAND.emerald}`, fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Sign in to access admin</a>
      </div>
    );
  }
  if (user.role !== "admin") {
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", color: "#e2e8f0", fontFamily: "var(--font-mono)" }}>
      ...
      <div style={{ borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "1.25rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}>
        ...
        <button
          onClick={openCreate}
          style={{ display: "flex", alignItems: "center", gap: "0.4rem", background: `${BRAND.emerald}`, color: "#0a0a0a", border: "none", borderRadius: "0.375rem", padding: "0.5rem 1rem", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer" }}
        >
      ...
      <div style={{ padding: "0.875rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
        {["", "meeting", "demo", "call", "event", "follow_up"].map(t => (
          <button key={t} onClick={() => setTypeFilter(t)}
            style={{ padding: "0.25rem 0.75rem", borderRadius: "999px", border: `1px solid ${typeFilter === t ? `${BRAND.emerald}` : "rgba(255,255,255,0.12)"}`, background: typeFilter === t ? emeraldAlpha(0.1) : "transparent", color: typeFilter === t ? `${BRAND.emerald}` : "#94a3b8", fontSize: "0.5625rem", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer" }}>
        ...
        {["", "scheduled", "confirmed", "completed", "cancelled"].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            style={{ padding: "0.25rem 0.75rem", borderRadius: "999px", border: `1px solid ${statusFilter === s ? STATUS_CONFIG[s as EventStatus]?.color ?? `${BRAND.emerald}` : "rgba(255,255,255,0.12)"}`, background: statusFilter === s ? "rgba(255,255,255,0.05)" : "transparent", color: statusFilter === s ? (STATUS_CONFIG[s as EventStatus]?.color ?? `${BRAND.emerald}`) : "#94a3b8", fontSize: "0.5625rem", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: "0.375rem", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "0.5rem", padding: "0.25rem", background: "rgba(255,255,255,0.025)" }}>
          {(["day", "week", "month"] as CalendarView[]).map(v => (
            <button key={v} onClick={() => setView(v)}
              style={{ border: "none", borderRadius: "0.35rem", background: view === v ? emeraldAlpha(0.15) : "transparent", color: view === v ? `${BRAND.emerald}` : "#94a3b8", padding: "0.35rem 0.75rem", fontSize: "0.5625rem", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer" }}>
              {v}
            </button>
          ))}
        </div>
          ...
          <>
              ...
              <div style={{ border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.75rem", overflow: "hidden", background: "rgba(255,255,255,0.015)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>
                  {monthDays.map(day => {
                    ...
                    return (
                      <div key={day.toISOString()} style={{ minHeight: 132, padding: "0.6rem", borderRight: day.getDay() === 6 ? "none" : "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)", background: today ? emeraldAlpha(0.045) : "transparent", opacity: activeMonth ? 1 : 0.35 }}>
                        <button onClick={() => { setCursorDate(day); setView("day"); }} style={{ background: today ? `${BRAND.emerald}` : "transparent", color: today ? "#0a0a0a" : "#94a3b8", border: "none", borderRadius: "999px", width: 24, height: 24, fontSize: "0.625rem", fontWeight: 800, cursor: "pointer", marginBottom: "0.4rem" }}>{day.getDate()}</button>
            ...
            {view === "week" && (
              <div style={{ border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.75rem", overflow: "hidden", background: "rgba(255,255,255,0.015)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "56px repeat(7, minmax(0, 1fr))", background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  <div />
                  {weekDays.map(day => <div key={day.toISOString()} style={{ padding: "0.65rem", textAlign: "center", borderLeft: "1px solid rgba(255,255,255,0.06)" }}><div style={{ color: "#64748b", fontSize: "0.55rem", fontWeight: 800, textTransform: "uppercase" }}>{DAY_LABELS[day.getDay()]}</div><button onClick={() => { setCursorDate(day); setView("day"); }} style={{ marginTop: "0.25rem", background: sameDay(day, new Date()) ? `${BRAND.emerald}` : "transparent", color: sameDay(day, new Date()) ? "#0a0a0a" : "#e2e8f0", border: "none", borderRadius: "999px", width: 26, height: 26, fontWeight: 800, cursor: "pointer" }}>{day.getDate()}</button></div>)}
                </div>
                {DAY_HOURS.map(hour => (
            ...
            {filtered.length === 0 && (
              <div style={{ marginTop: "1rem", border: "1px dashed rgba(255,255,255,0.12)", borderRadius: "0.75rem", padding: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem", background: "rgba(255,255,255,0.015)" }}>
                ...
                <button onClick={openCreate} style={{ background: emeraldAlpha(0.1), color: `${BRAND.emerald}`, border: `1px solid ${emeraldAlpha(0.3)}`, borderRadius: "0.375rem", padding: "0.5rem 1.25rem", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer", whiteSpace: "nowrap" }}>
                  + Create Event
                </button>
              </div>
            )}
          ...
          <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "0.75rem", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflow: "auto" }}>
            ...
            <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "flex-end", gap: "0.75rem" }}>
              ...
              <button onClick={handleSave} disabled={saving}
                style={{ background: saving ? emeraldAlpha(0.4) : `${BRAND.emerald}`, color: "#0a0a0a", border: "none", borderRadius: "0.375rem", padding: "0.5rem 1.25rem", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: saving ? "not-allowed" : "pointer" }}>
          ...
          <div style={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "0.75rem", width: "100%", maxWidth: "420px", margin: "1rem" }}
            ...
            <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "flex-end", gap: "0.75rem" }}>
              ...
              <button
                ...
                style={{ background: rescheduleMutation.isPending ? emeraldAlpha(0.4) : `${BRAND.emerald}`, color: "#0a0a0a", border: "none", borderRadius: "0.375rem", padding: "0.5rem 1.25rem", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: rescheduleMutation.isPending ? "not-allowed" : "pointer" }}>
