...
type Props = {
  ...
  emptyPending?: React.ReactNode;
  compact?: boolean;
};

export default function OutreachDraftQueue({
  audience = "all",
  emptyPending,
  compact = false,
}: Props) {
  ...
  const { data: drafts = [], isLoading } = trpc.admin.getDrafts.useQuery(
    { statuses: statusFilter, audience },
  ...
  const { data: allSendableDrafts = [] } = trpc.admin.getDrafts.useQuery(
    { statuses: ["pending", "approved"], audience },
  ...
  const recipientLabel = (entry: DraftEntry) =>
    entry.recipient?.company ?? entry.prospect?.company ?? "Unknown";

  const recipientContact = (entry: DraftEntry) =>
    entry.recipient?.contactName ?? entry.prospect?.contactName;

  const recipientEmail = (entry: DraftEntry) =>
    entry.recipient?.contactEmail ?? entry.prospect?.contactEmail;
  ...
  return (
    <div style={{ color: "#ececec" }}>
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontSize: "0.875rem", fontWeight: 600, color: "#ececec", margin: 0 }}>
              Ready to send
              {selectedIds.size > 0 && (
                <span style={{ color: "rgba(255,255,255,0.45)", fontWeight: 400 }}> · {selectedIds.size} selected</span>
              )}
            </p>
          </div>
      ...
      {activeTab !== "sent" && drafts.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem", padding: "0.625rem 0.875rem", background: "#1C1E22", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.375rem" }}>
          <input
            type="checkbox"
      ...
      {isLoading ? (
        <div style={{ textAlign: "center", padding: compact ? "2rem 0" : "4rem 0", color: "rgba(255,255,255,0.30)", fontSize: "0.875rem" }}>Loading drafts…</div>
      ) : drafts.length === 0 ? (
        <div style={{ textAlign: "center", padding: "2rem 0", color: "rgba(255,255,255,0.30)", fontSize: "0.875rem" }}>
          {activeTab === "pending" ? (
            emptyPending ?? <p>No drafts pending review.</p>
          ) : activeTab === "approved" ? (
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.875rem 1rem" }}>
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 600, fontSize: "0.9375rem", color: "#ececec" }}>{recipientLabel(entry)}</span>
                      {sourceLabel(entry) && (
                        <span style={{ fontSize: "0.6875rem", color: "#64748b", border: "1px solid rgba(255,255,255,0.12)", padding: "0.1rem 0.35rem", borderRadius: "0.25rem" }}>
                          {sourceLabel(entry)}
                        </span>
                      )}
                      {recipientContact(entry) && (
                        <span style={{ fontSize: "0.8125rem", color: "#64748b" }}>→ {recipientContact(entry)}</span>
                      )}
                      {recipientEmail(entry) && (
                        <span style={{ fontSize: "0.8125rem", color: "rgba(255,255,255,0.30)" }}>{recipientEmail(entry)}</span>
                      )}
                ...
                {isExpanded && (
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    {entry.draft.agentReasoning && (
                      <div style={{ fontSize: "0.8125rem", color: "#64748b", background: "#1C1E22", borderRadius: "0.375rem", padding: "0.625rem 0.875rem", border: "1px solid rgba(255,255,255,0.08)" }}>
                    ...
                    {isEditing ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        <div>
                          ...
                          <Input value={editSubject} onChange={(e) => setEditSubject(e.target.value)} style={{ fontSize: "0.875rem" }} />
                        </div>
                        <div>
                          <label style={{ fontSize: "0.75rem", color: "#64748b", display: "block", marginBottom: "0.25rem" }}>Body</label>
                          <Textarea value={editBody} onChange={(e) => setEditBody(e.target.value)} rows={10} style={{ fontSize: "0.875rem", fontFamily: "monospace" }} />
                        </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                        ...
                        <pre style={{ fontSize: "0.875rem", whiteSpace: "pre-wrap", fontFamily: "inherit", background: "#1C1E22", borderRadius: "0.375rem", padding: "0.875rem", border: "1px solid rgba(255,255,255,0.08)", lineHeight: 1.6, color: "#ececec", margin: 0 }}>
                          {entry.draft.body}
                        </pre>
                        {activeTab !== "sent" && (
                          <div style={{ display: "flex", gap: "0.5rem" }}>
